...
#完成工程类型的识别
set proj_work_path "../../work"
set vivado_runname [glob -nocomplain ../../work/*.runs]
set qaurtus_qpfname [glob -nocomplain ../../work/*.qpf]
...
set outputfile_name [glob -nocomplain ../../work/program_debug_file]

if {[llength $outputfile_name] > 0 } {
    #考虑是不是删除之前的文件夹下的bit bin sof jic 文件
    puts $outputfile_name
} else {
    file mkdir ../../program_debug_file
...
if {$project_type == "vivado"} {
    ...
    if {[file exists "${vivadoimplname}/debug_nets.ltx"]} {
        file copy -force "${vivadoimplname}/debug_nets.ltx" ../../program_debug_file
    } else {
    ...
    if {[file exists "${vivadoimplname}/${top_name}.bit"]} {
        file copy -force "${vivadoimplname}/${top_name}.bit" ../../program_debug_file
    } else {
    ...
    if {[file exists "${vivadoimplname}/${top_name}.bin"]} {
        file copy -force "${vivadoimplname}/${top_name}.bin" ../../program_debug_file
    } else {
        puts "${top_name}.bin file does not exist"
    }
    #copy $top_name.prm
    if {[file exists "${vivadoimplname}/${top_name}.prm"]} {
        file copy -force "${vivadoimplname}/${top_name}.prm" ../../program_debug_file
    } else {
    ...
    if {[file exists "${vivadoimplname}/${top_name}.mcs"]} {
        file copy -force "${vivadoimplname}/${top_name}.mcs" ../../program_debug_file
    } else {
    ...
    if {[file exists "${proj_work_path}/${qaurtus_proj_name}.sof"]} {
        file copy -force "${proj_work_path}/${qaurtus_proj_name}.sof" ../../program_debug_file
    } else {
    ...
    if {[file exists "${proj_work_path}/${qaurtus_proj_name}.jic"]} {
        file copy -force "${proj_work_path}/${qaurtus_proj_name}.jic" ../../program_debug_file
    } else {
        puts "${qaurtus_proj_name}.jic file does not exist"
    }
    #copy .rpd file 
    if {[file exists "${proj_work_path}/${qaurtus_proj_name}_auto.rpd"]} {
        file copy -force "${proj_work_path}/${qaurtus_proj_name}_auto.rpd" ../../program_debug_file
    } else {
        puts "${qaurtus_proj_name}_auto.rpd file does not exist"
    }
    #copy .map file 
    if {[file exists "${proj_work_path}/${qaurtus_proj_name}.map"]} {
        file copy -force "${proj_work_path}/${qaurtus_proj_name}.map" ../../program_debug_file
    } else {
    ...
    if {[llength $signaltap_file_path] > 0} {
        ...
        if {[llength $signaltap_name] > 0} {
            file copy -force $signaltap_name ../../program_debug_file
        }
    } else {
